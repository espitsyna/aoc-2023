...
const go = ([i, j], direction, map) => {
    ...
    switch (data[i][j]) {
        ...
        case '|':
            ...
            return [map, ...go(t1, UP, newMap), ...go(t2, DOWN, newMap)];
        case '-':
            ...
            return [map, ...go(t3, RIGHT, newMap), ...go(t4, LEFT, newMap)];
        default:
            return [map];
...
const result = go([0, 0], RIGHT, newMap);
const sum = data.reduce((acc, line, i) => acc + line.reduce((acc, _, j) => acc + (result.some(r => r[i][j].length) ? 1 : 0), 0), 0);
console.log(sum);