...
const getFalling = removed => Object.values(data).filter(({ key, supportedBy }) => !removed.includes(key) && supportedBy.length && supportedBy.every(key => removed.includes(key))).map(({ key }) => key);

const amount = Object.keys(data).reduce((acc, key) => {
    let removed = [key];
    let falling = getFalling(removed);
    while (falling.length) {
        removed = [...removed, ...falling];
        falling = getFalling(removed);
    }
    return acc + removed.length - 1;
}, 0);
